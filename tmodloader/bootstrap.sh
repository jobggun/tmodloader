...
if [ -z "$WORLD_FILENAME" ]; then
  ...
  exec dotnet tModLoader.dll -server -config "$CONFIGPATH/$CONFIG_FILENAME" -logpath "$LOGPATH" "$@"
else
  echo "Environment WORLD_FILENAME specified"
  if [ -f "$WORLD_PATH" ]; then
    echo "Loading to world $WORLD_FILENAME..."
    exec dotnet tModLoader.dll -server -config "$CONFIGPATH/$CONFIG_FILENAME" -logpath "$LOGPATH" -world "$WORLD_PATH" "$@"
  else
